...
import net.mcreator.aurorasmpnew.procedures.LivroAuroraAbertoRightclickedProcedure;
import net.mcreator.aurorasmpnew.item.renderer.LivroAuroraAbertoItemRenderer;
...
public class LivroAuroraAbertoItem extends Item implements IAnimatable {
	public AnimationFactory factory = GeckoLibUtil.createFactory(this);
	public String animationprocedure = "empty";
	public static ItemTransforms.TransformType transformType;

	public LivroAuroraAbertoItem() {
	...
	public void initializeClient(Consumer<IItemRenderProperties> consumer) {
		...
		consumer.accept(new IItemRenderProperties() {
			private final BlockEntityWithoutLevelRenderer renderer = new LivroAuroraAbertoItemRenderer();
	...
	protected <P extends IAnimatable> void customInstructionListener(CustomInstructionKeyframeEvent<P> event) {
		...
		if (!(ister instanceof LivroAuroraAbertoItemRenderer))
			return;
		LivroAuroraAbertoItemRenderer renderer = (LivroAuroraAbertoItemRenderer) ister;
		instructionTokens.stream().filter(tks -> !tks.isEmpty()).forEach(tks -> this.interpretFirstPersonInstructions(tks, renderer));
	}

	protected void interpretFirstPersonInstructions(List<String> tokens, LivroAuroraAbertoItemRenderer renderer) {
	...
	private <P extends Item & IAnimatable> PlayState idlePredicate(AnimationEvent<P> event) {
		if (this.transformType != null ? true : false) {
			if (this.animationprocedure.equals("empty")) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("aberto", EDefaultLoopTypes.LOOP));
	...
	public void setupAnimationState(LivroAuroraAbertoItemRenderer renderer, ItemStack stack, PoseStack matrixStack, float aimProgress) {
	}
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player entity, InteractionHand hand) {
		...
		LivroAuroraAbertoRightclickedProcedure.execute(entity);
		return ar;
	}
}
